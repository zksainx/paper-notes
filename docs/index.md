# 欢迎来到论文笔记

这是一个用于记录和整理论文阅读笔记的个人知识库。

## 🎯 网站简介

本站点记录了我在人工智能、机器学习等领域的论文阅读笔记。每篇笔记包含论文的核心思想、方法总结、实验结果分析以及个人思考。

大多数性能导向的系统研究可以归纳为“固定资源下的再分配优化”，并伴随着清晰的 trade-off。

如果系统研究关注的是“资源空间中的最优分配”，那么算法研究关注的是“函数空间中的最优构造”。

## 📚 内容分类

网站按研究领域组织论文笔记：

### [Simulation](simulation/index.md)
Papers on performance modeling, simulation frameworks, and analytical models for ML systems.

### [Tensor Program Optimization](tensor-program-optimization/index.md)
Papers on automated tensor program optimization through compiler techniques, ML-guided search, and performance modeling for deep learning workloads.

### [Automated Kernel Generation](automated-kernel-generation/index.md)
Papers on LLM-driven generation, profiling-guided refinement, and automated optimization of GPU kernels and low-level code.

### [Intra-GPU Spatial Multiplexing](intra-gpu-multiplexing/index.md)
Papers on GPU spatial multiplexing techniques for efficient LLM serving, including prefill-decode disaggregation and dynamic resource partitioning.

### [Cluster Performance Modeling](cluster-performance-modeling/index.md)
Papers on probabilistic runtime modeling, tail-latency analysis, and variability-aware scheduling for large-scale distributed training clusters.

### [Speculative Decoding](speculative-decoding/index.md)
Papers on lossless LLM inference acceleration through efficient drafting, parallel verification, dynamic candidate construction, and serving-aware scheduling.

### [MTP](mtp/index.md)
Papers on multi-token prediction as a training objective, model-internal future-token capability, and its use for reasoning or inference acceleration.

### [LLM Infrastructure](llm-infrastructure/index.md)
OSDI and SOSP 2025-2026 papers on large-model training, serving, memory, agents, and production reliability.

## 🔍 如何使用

- **浏览分类**：点击顶部导航栏选择感兴趣的研究领域
- **搜索论文**：使用右上角的搜索框快速查找特定论文或关键词
- **阅读笔记**：每篇笔记包含论文摘要、核心思想、方法介绍和个人理解

## 📝 笔记模板

每篇论文笔记通常包含以下内容：

- **论文基本信息**（标题、作者、会议/期刊、年份）
- **摘要**：论文要解决的问题和主要贡献
- **核心思想**：论文的关键创新点
- **方法**：技术细节和实现方式
- **实验结果**：主要实验结论
- **个人笔记**：个人思考、疑问和延伸阅读

## 🚀 更新日志

本网站会持续更新，记录最新阅读的论文。欢迎访问 [GitHub 仓库](https://github.com/zksainx/paper-notes) 查看更新历史。

---

*最后更新：2025年*
